...
CREATE TABLE Album (
    ...
    CONSTRAINT fk_album_musician FOREIGN KEY (musician_id) REFERENCES Musician(id) ON DELETE CASCADE,
...
CREATE TABLE Song (
    id SERIAL PRIMARY KEY,
    title VARCHAR(150) NOT NULL,
    duration INT NOT NULL CHECK (duration >= 0),  -- Duration as unsigned int, ensuring positive values
...
CREATE INDEX idx_album_musician ON Album (musician_id);
